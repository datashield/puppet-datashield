...
define datashield::server_package($r_path = '/usr/bin/R', $opal_password = 'password', $opal_url = 'http://localhost:8080',
  ...
  exec { "install_datashield_package_${name}":
    command   => "${r_path} -e \"library(opaladmin); o<-opal.login('administrator', '${opal_password}', url='${opal_url}'); dsadmin.install_package(o, '${name}'); dsadmin.remove_package(o, '${name}'); dsadmin.install_package(o, '${name}', ref='${ref}'); dsadmin.set_package_methods(o, pkg='${name}')\" | grep 'TRUE' ",
